...
export function SidebarFinal({ content }) {
  const links = [
    {
      label: "Dashboard",
      href: "#",
  ...
  return (
    (<div
      className={cn(
        "min-h-screen w-h-screen rounded-md flex flex-col md:flex-row bg-gray-100  w-screen flex-1 mx-auto border border-neutral-200",
        "h-screen"
      )}>
...
export const Logo = () => {
  return (
    (<a
      ...
      <div
        className="h-5 w-6 bg-primary  rounded-br-lg rounded-tr-sm rounded-tl-lg rounded-bl-sm flex-shrink-0" />
      <motion.span
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
...
const Dashboard = ({ content }) => {
  return (
    (<div className="flex flex-1 overflow-y-scroll h-full">
      <div
        className="p-2 md:p-10 rounded-tl-2xl border border-back bg-white flex flex-col gap-2 flex-1 w-full h-min min-h-screen justify-center">
        {content}
      </div>
    </div>)
  );
...
export const MobileSidebar = ({
  ...
  return (<>
    <div
      className={cn(
        "h-10 px-4 py-4 flex flex-row md:hidden items-center justify-between bg-back w-full"
      )}
      {...props}>
